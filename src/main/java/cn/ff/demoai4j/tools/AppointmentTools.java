package cn.ff.demoai4j.tools;

import cn.ff.demoai4j.entity.Appointment;
import cn.ff.demoai4j.mapper.AppointmentMapper;
...
public class AppointmentTools {
    ...
    public String bookAppointment(Appointment appointment) {
        //查找数据库中是否包含对应的预约记录
        Appointment appointmentDB = appointmentMapper.getById(appointment.getId());
    ...
    public String cancelAppointment(Appointment appointment) {
        ...
        if (appointmentDB != null) {
            //删除预约记录
            if (appointmentMapper.deleteById(appointmentDB.getId()) > 0) {
                return "取消预约成功";
            } else {
                return "取消预约失败";
            }
    ...
    public boolean queryDepartment(@P(value = "科室名称") String name, @P(value = "日期") String date,
                                   @P(value = "时间，可选值：上午、下午") String time, @P(value = "医生名称", required = false) String doctorName) {
